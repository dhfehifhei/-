-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v8
-- ==========================================================
...
local Settings = {
    ...
    DroneMobileControls = true,
    DroneMobileScale = 1.0,
}

-- ==========================================================
--  Apply colors to WindUI theme
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "8.0" },
...
local function importConfigFromJSON(json, saveAs)
    if not (writefile and isfolder) then return false end    local ok, data = pcall(function() return HttpService:JSONDecode(json) end)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "8.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "8.0" }, settings = data }
    end
...
-- ==========================================================
--  DRONE
--  Реалистичная модель типа DJI Mavic:
--  центральный корпус, камера-подвес, складные лучи с моторами
--  и лопастями, посадочные шасси, LED-огни, антенна.
-- ==========================================================
local Drone = {
    model        = nil,
    base         = nil,
...
local function buildDroneModel()
    ...
    model.Name = "MellDrone"

    local DARK   = Color3.fromRGB(35, 38, 45)
    local DARKER = Color3.fromRGB(20, 22, 28)
    local GRAY   = Color3.fromRGB(80, 84, 92)
    local ACCENT = Color3.fromRGB(255, 40, 90)

    -- ============ ЦЕНТРАЛЬНЫЙ КОРПУС ============
    local core = Instance.new("Part")
    core.Name = "Core"
    core.Size = Vector3.new(1.6, 0.55, 2.2)
    ...
    -- верхняя крышка с полосой
    local topCover = Instance.new("Part")
    topCover.Name = "TopCover"
    topCover.Size = Vector3.new(1.4, 0.08, 2.0)
    topCover.Color = DARKER
    topCover.Material = Enum.Material.SmoothPlastic
    topCover.Anchored = true
    topCover.CanCollide = false
    topCover.Massless = true
    topCover.Parent = model
    topCover.CFrame = core.CFrame * CFrame.new(0, 0.32, 0)

    -- полоса акцента на корпусе
    local stripe = Instance.new("Part")
    stripe.Name = "Stripe"
    stripe.Size = Vector3.new(1.62, 0.06, 0.14)
    stripe.Color = ACCENT
    ...
    stripe.Parent = model
    stripe.CFrame = core.CFrame * CFrame.new(0, 0.28, 0)

    -- стекло-«глаз» спереди
    local frontGlass = Instance.new("Part")
    frontGlass.Name = "FrontGlass"
    frontGlass.Shape = Enum.PartType.Ball
    frontGlass.Size = Vector3.new(0.75, 0.55, 0.6)
    frontGlass.Color = Color3.fromRGB(15, 18, 25)
    frontGlass.Material = Enum.Material.Glass
    frontGlass.Transparency = 0.15
    frontGlass.Anchored = true
    frontGlass.CanCollide = false
    frontGlass.Massless = true
    frontGlass.Parent = model
    frontGlass.CFrame = core.CFrame * CFrame.new(0, 0.05, -0.9)

    -- ============ КАМЕРА-ПОДВЕС (снизу) ============
    local gimbalYoke = Instance.new("Part")
    gimbalYoke.Name = "GimbalYoke"
    gimbalYoke.Size = Vector3.new(0.5, 0.25, 0.5)
    gimbalYoke.Color = GRAY
    gimbalYoke.Material = Enum.Material.SmoothPlastic
    gimbalYoke.Anchored = true
    gimbalYoke.CanCollide = false
    gimbalYoke.Massless = true
    gimbalYoke.Parent = model
    gimbalYoke.CFrame = core.CFrame * CFrame.new(0, -0.4, -0.5)

    local camBall = Instance.new("Part")
    camBall.Name = "CameraBall"
    camBall.Shape = Enum.PartType.Ball
    camBall.Size = Vector3.new(0.55, 0.55, 0.55)
    camBall.Color = DARKER
    camBall.Material = Enum.Material.SmoothPlastic
    camBall.Anchored = true
    camBall.CanCollide = false
    camBall.Massless = true
    camBall.Parent = model
    camBall.CFrame = core.CFrame * CFrame.new(0, -0.55, -0.5)

    local camLens = Instance.new("Part")
    camLens.Name = "CameraLens"
    camLens.Shape = Enum.PartType.Cylinder
    camLens.Size = Vector3.new(0.06, 0.32, 0.32)
    camLens.Color = Color3.fromRGB(10, 12, 18)
    camLens.Material = Enum.Material.Glass
    camLens.Transparency = 0.1
    camLens.Anchored = true
    camLens.CanCollide = false
    camLens.Massless = true
    camLens.Parent = model
    camLens.CFrame = core.CFrame * CFrame.new(0, -0.55, -0.78) * CFrame.Angles(0, 0, math.rad(90))

    -- ============ ПОСАДОЧНЫЕ ШАССИ (4 ножки) ============
    local legOffsets = {
        { pos = Vector3.new( 0.55, -0.75,  0.7), flip = 0 },
        { pos = Vector3.new(-0.55, -0.75,  0.7), flip = 0 },
        { pos = Vector3.new( 0.55, -0.75, -0.7), flip = 0 },
        { pos = Vector3.new(-0.55, -0.75, -0.7), flip = 0 },
    }
    for i, data in ipairs(legOffsets) do
        ...
        foot.Name = "Foot" .. i
        foot.Size = Vector3.new(0.18, 0.06, 0.35)
        foot.Color = DARKER
        ...
        foot.Massless = true
        foot.Parent = model
        foot.CFrame = core.CFrame * CFrame.new(data.pos.X, data.pos.Y - 0.25, data.pos.Z)
    end

    -- ============ 4 ЛУЧА + МОТОРЫ + ЛОПАСТИ ============
    local armData = {
        -- передние/задние лучи крест-накрест
        { off = Vector3.new( 1.4, 0.05,  1.4), angle = math.rad(45)  },
        { off = Vector3.new(-1.4, 0.05,  1.4), angle = math.rad(-45) },
        { off = Vector3.new( 1.4, 0.05, -1.4), angle = math.rad(-45) },
        { off = Vector3.new(-1.4, 0.05, -1.4), angle = math.rad(45)  },
    }
    ...
    for i, data in ipairs(armData) do
        ...
        -- складной луч (длинная тонкая труба)
        local arm = Instance.new("Part")
        arm.Name = "Arm" .. i
        arm.Size = Vector3.new(2.1, 0.16, 0.28)
        arm.Color = DARK
        ...
        arm.Massless = true
        arm.Parent = model
        -- размещаем между core и мотором, поворачиваем по диагонали
        local armAngle = math.atan2(off.X, off.Z)
        arm.CFrame = core.CFrame
            * CFrame.new(off.X / 2, off.Y, off.Z / 2)
            * CFrame.Angles(0, armAngle, 0)

        -- хомут на луче (деталь)
        local armWrap = Instance.new("Part")
        armWrap.Name = "ArmWrap" .. i
        armWrap.Size = Vector3.new(0.32, 0.24, 0.4)
        armWrap.Color = DARKER
        armWrap.Material = Enum.Material.SmoothPlastic
        armWrap.Anchored = true
        armWrap.CanCollide = false
        armWrap.Massless = true
        armWrap.Parent = model
        armWrap.CFrame = core.CFrame
            * CFrame.new(off.X / 2, off.Y, off.Z / 2)
            * CFrame.Angles(0, armAngle, 0)

        -- мотор (цилиндр вертикальный)
        local motor = Instance.new("Part")
        motor.Name = "Motor" .. i
        motor.Shape = Enum.PartType.Cylinder
        motor.Size = Vector3.new(0.45, 0.5, 0.5)
        motor.Color = DARK
        ...
        -- верхняя крышка мотора (неон акцент чередуется)
        local cap = Instance.new("Part")
        cap.Name = "MotorCap" .. i
        cap.Shape = Enum.PartType.Cylinder
        cap.Size = Vector3.new(0.1, 0.44, 0.44)
        cap.Material = Enum.Material.Neon
        cap.Color = (i % 2 == 0) and ACCENT or Color3.fromRGB(80, 200, 255)
        cap.Anchored = true
        cap.CanCollide = false
        cap.Massless = true
        cap.Parent = model
        cap.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.26, 0)) * CFrame.Angles(0, 0, math.rad(90))

        -- лопасти (2 крест-накрест)
        local blade1 = Instance.new("Part")
        blade1.Name = "Blade1_" .. i
        blade1.Size = Vector3.new(1.7, 0.03, 0.16)
        blade1.Color = Color3.fromRGB(230, 232, 240)
        blade1.Material = Enum.Material.SmoothPlastic
        blade1.Transparency = 0.1
        ...
        blade2.Name = "Blade2_" .. i
        blade2.Size = Vector3.new(0.16, 0.03, 1.7)
        blade2.Color = Color3.fromRGB(230, 232, 240)
        blade2.Material = Enum.Material.SmoothPlastic
        blade2.Transparency = 0.1
        ...
        -- кольцо-размытие (вращающийся диск)
        local blur = Instance.new("Part")
        blur.Name = "BladeBlur" .. i
        blur.Shape = Enum.PartType.Cylinder
        blur.Size = Vector3.new(0.02, 1.65, 1.65)
        blur.Material = Enum.Material.Neon
        blur.Color = Color3.fromRGB(200, 200, 220)
    ...
    -- ============ LED-ОГНИ ============
    -- передние — красные
    for _, pos in ipairs({ Vector3.new( 0.4, 0.1, -1.05), Vector3.new(-0.4, 0.1, -1.05) }) do
        local led = Instance.new("Part")
        led.Name = "FrontLED"
        led.Shape = Enum.PartType.Ball
        led.Size = Vector3.new(0.14, 0.14, 0.14)
        led.Material = Enum.Material.Neon
        led.Color = Color3.fromRGB(255, 60, 60)
        led.Anchored = true
        led.CanCollide = false
        led.Massless = true
        led.Parent = model
        led.CFrame = core.CFrame * CFrame.new(pos)
    end
    ...
    for _, pos in ipairs({ Vector3.new( 0.4, 0.1, 1.05), Vector3.new(-0.4, 0.1, 1.05) }) do
        ...
        led.Size = Vector3.new(0.14, 0.14, 0.14)
        led.Material = Enum.Material.Neon
        led.Color = Color3.fromRGB(60, 255, 100)
        led.Anchored = true
        led.CanCollide = false
        led.Massless = true
        led.Parent = model
    ...
    antenna.CFrame = core.CFrame * CFrame.new(0, 0.6, 0.85)

    local antennaTip = Instance.new("Part")
    antennaTip.Name = "AntennaTip"
    antennaTip.Shape = Enum.PartType.Ball
    antennaTip.Size = Vector3.new(0.12, 0.12, 0.12)
    antennaTip.Material = Enum.Material.Neon
    antennaTip.Color = Color3.fromRGB(255, 180, 40)
    antennaTip.Anchored = true
    antennaTip.CanCollide = false
    antennaTip.Massless = true
    antennaTip.Parent = model
    antennaTip.CFrame = core.CFrame * CFrame.new(0, 0.88, 0.85)
...
local function spawnExplosion(position)
    ...
    ball.Color = Color3.fromRGB(255, 180, 40)
    ball.Anchored = true
    ...
    TweenService:Create(ball, TweenInfo.new(0.45), {Color = Color3.fromRGB(255, 80, 20)}):Play()
    ...
    spark.Speed = NumberRange.new(20, 40)
...
local function knockbackPlayer(player, fromPos)
    ...
    dir = dir.Unit
    hrp.AssemblyLinearVelocity = dir * 80 + Vector3.new(0, 40, 0)
end
        ...
        for _, remote in pairs(game:GetService("ReplicatedStorage"):GetDescendants()) do
            if remote:IsA("RemoteEvent") then
                ...
                if string.find(n, "damage") or string.find(n, "hit") or string.find(n, "attack") then
                    pcall(function() remote:FireServer(player, Settings.DroneDamage) end)
                end
            end
        end
    end)
...
local function onDroneTouched(hit)
    if Drone.destroyed then return end
    if not Drone.base then return end
    ...
    local position = Drone.base.Position
    spawnExplosion(position)
    knockbackPlayer(plr, position)
    tryDamagePlayer(plr)

    Drone.lastHit = plr.Name
    Drone.hitTimer = 3

    WindUI:Notify({
        Title = "Drone",
        Content = "BOOM! Hit: " .. plr.Name,
        Icon = "bomb",
        Duration = 4,
    })

    if Settings.DroneAutoDestroy then
        Drone.destroyed = true
        Drone.deathTimer = 1.2
    end
end
...
function Drone:Start()
    ...
    local char = LocalPlayer.Character
    if not char then
        WindUI:Notify({ Title = "Drone", Content = "No character", Icon = "x", Duration = 3 })
        return
    end
    local hum = getHumanoid(char)
    local hrp = getRoot(char)
    if not hum or not hrp then
        WindUI:Notify({ Title = "Drone", Content = "No humanoid/root", Icon = "x", Duration = 3 })
        return
    end
    ...
    self.droneCF = hrp.CFrame * CFrame.new(0, 3, 0)
    ...
    self.base.Touched:Connect(onDroneTouched)
    ...
    if Settings.DroneCamera == "FPV" and Settings.DroneFPVEffects then
        Camera.FieldOfView = Settings.DroneFPVFOV
    end

    self.active = true
    WindUI:Notify({ Title = "Drone", Content = "Drone launched!", Icon = "plane", Duration = 3 })
end
...
function Drone:Stop()
    ...
    -- скрыть мобильные контролы
    if Drone.MobileControls then
        Drone.MobileControls:SetVisible(false)
    end

    WindUI:Notify({ Title = "Drone", Content = "Drone stopped", Icon = "x", Duration = 2 })
end
...
local vTopGrad = Instance.new("UIGradient")
vTopGrad.Rotation = 90
vTopGrad.Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 0),
    NumberSequenceKeypoint.new(1, 1),
})
vTopGrad.Parent = vTop

local vBottomGrad = Instance.new("UIGradient")
vBottomGrad.Rotation = 270
vBottomGrad.Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 0),
    NumberSequenceKeypoint.new(1, 1),
})
vBottomGrad.Parent = vBottom

local vLeftGrad = Instance.new("UIGradient")
vLeftGrad.Rotation = 0
vLeftGrad.Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 0),
    NumberSequenceKeypoint.new(1, 1),
})
vLeftGrad.Parent = vLeft

local vRightGrad = Instance.new("UIGradient")
vRightGrad.Rotation = 180
vRightGrad.Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 0),
    NumberSequenceKeypoint.new(1, 1),
})
vRightGrad.Parent = vRight
...
local fpvCenterDot = Instance.new("Frame")
fpvCenterDot.BackgroundColor3 = Color3.fromRGB(255, 40, 90)
fpvCenterDot.BorderSizePixel = 0
fpvCenterDot.Size = UDim2.fromOffset(4, 4)
fpvCenterDot.Position = UDim2.fromOffset(18, 18)
fpvCenterDot.ZIndex = 51
fpvCenterDot.Parent = fpvCrosshair
...
-- ==========================================================
--  МОБИЛЬНОЕ УПРАВЛЕНИЕ ДРОНОМ (джойстики + кнопки)
-- ==========================================================
local MobileDroneControls = {}
MobileDroneControls.__index = MobileDroneControls

-- общий "ввод" от мобильных контролов
local mobileInput = {
    moveX = 0, moveY = 0,       -- левый стик: X = strafe, Y = forward
    lookX = 0, lookY = 0,       -- правый стик: X = yaw, Y = pitch
    up = false, down = false,   -- кнопки высоты
    boom = false,               -- кнопка "взрыв"
}

function MobileDroneControls.new(parent)
    local self = setmetatable({}, MobileDroneControls)

    self.gui = Instance.new("ScreenGui")
    self.gui.Name = "MellDroneMobileControls"
    ...
    local scaleVal = Settings.DroneMobileScale or 1
    local function px(n) return math.floor(n * scaleVal) end

    -- ============ ЛЕВЫЙ СТИК (движение) ============
    self.leftBase = Instance.new("Frame")
    self.leftBase.Name = "LeftStickBase"
    self.leftBase.Size = UDim2.fromOffset(px(140), px(140))
    self.leftBase.Position = UDim2.new(0, px(30), 1, -px(170))
    self.leftBase.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
    self.leftBase.BackgroundTransparency = 0.45
    self.leftBase.BorderSizePixel = 0
    self.leftBase.ZIndex = 100
    self.leftBase.Parent = self.gui
    local leftCorner = Instance.new("UICorner", self.leftBase)
    leftCorner.CornerRadius = UDim.new(1, 0)
    local leftStroke = Instance.new("UIStroke", self.leftBase)
    leftStroke.Color = Color3.fromRGB(255, 40, 90)
    leftStroke.Thickness = 2
    leftStroke.Transparency = 0.3

    self.leftKnob = Instance.new("Frame")
    self.leftKnob.Name = "LeftStickKnob"
    self.leftKnob.Size = UDim2.fromOffset(px(60), px(60))
    self.leftKnob.Position = UDim2.new(0.5, -px(30), 0.5, -px(30))
    self.leftKnob.BackgroundColor3 = Color3.fromRGB(255, 40, 90)
    self.leftKnob.BackgroundTransparency = 0.1
    self.leftKnob.BorderSizePixel = 0
    self.leftKnob.ZIndex = 101
    self.leftKnob.Parent = self.leftBase
    local leftKnobCorner = Instance.new("UICorner", self.leftKnob)
    leftKnobCorner.CornerRadius = UDim.new(1, 0)

    -- ============ ПРАВЫЙ СТИК (камера) ============
    self.rightBase = Instance.new("Frame")
    self.rightBase.Name = "RightStickBase"
    self.rightBase.Size = UDim2.fromOffset(px(140), px(140))
    self.rightBase.Position = UDim2.new(1, -px(170), 1, -px(170))
    self.rightBase.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
    self.rightBase.BackgroundTransparency = 0.45
    self.rightBase.BorderSizePixel = 0
    self.rightBase.ZIndex = 100
    self.rightBase.Parent = self.gui
    local rightCorner = Instance.new("UICorner", self.rightBase)
    rightCorner.CornerRadius = UDim.new(1, 0)
    local rightStroke = Instance.new("UIStroke", self.rightBase)
    rightStroke.Color = Color3.fromRGB(80, 200, 255)
    rightStroke.Thickness = 2
    rightStroke.Transparency = 0.3

    self.rightKnob = Instance.new("Frame")
    self.rightKnob.Name = "RightStickKnob"
    self.rightKnob.Size = UDim2.fromOffset(px(60), px(60))
    self.rightKnob.Position = UDim2.new(0.5, -px(30), 0.5, -px(30))
    self.rightKnob.BackgroundColor3 = Color3.fromRGB(80, 200, 255)
    self.rightKnob.BackgroundTransparency = 0.1
    self.rightKnob.BorderSizePixel = 0
    self.rightKnob.ZIndex = 101
    self.rightKnob.Parent = self.rightBase
    local rightKnobCorner = Instance.new("UICorner", self.rightKnob)
    rightKnobCorner.CornerRadius = UDim.new(1, 0)

    -- ============ КНОПКИ ВВЕРХ/ВНИЗ ============
    local function mkButton(name, label, xOff, yOff, color)
        local btn = Instance.new("TextButton")
        btn.Name = name
        btn.Size = UDim2.fromOffset(px(70), px(70))
        btn.Position = UDim2.new(1, xOff, 1, yOff)
    ...
    self.upBtn    = mkButton("UpBtn",    "^", -px(230), -px(260), Color3.fromRGB(60, 180, 90))
    self.downBtn  = mkButton("DownBtn",  "v", -px(230), -px(170), Color3.fromRGB(180, 90, 60))
    self.boomBtn  = mkButton("BoomBtn",  "!", -px(230), -px(80),  Color3.fromRGB(220, 40, 60))
    self.boomBtn.TextSize = px(34)

    -- ============ ОБРАБОТКА СТИКОВ ============
    local function setupStick(base, knob, onUpdate)
        local active = false
        ...
        local function update(px, py)
            ...
            local delta = Vector2.new(px, py) - center
            local maxR = base.AbsoluteSize.X / 2
            if delta.Magnitude > maxR then
                delta = delta.Unit * maxR
            end
            local norm = delta / maxR
            knob.Position = UDim2.new(0.5, -knob.AbsoluteSize.X/2 + delta.X,
                                       0.5, -knob.AbsoluteSize.Y/2 + delta.Y)
            onUpdate(norm.X, norm.Y)
        end
        ...
        UserInputService.InputEnded:Connect(function(input)
            if active and (input == activeInput or input.UserInputType == activeInput.UserInputType) then
                active = false
                activeInput = nil
                knob.Position = UDim2.new(0.5, -knob.AbsoluteSize.X/2, 0.5, -knob.AbsoluteSize.Y/2)
                onUpdate(0, 0)
            end
        end)
    end

    setupStick(self.leftBase, self.leftKnob, function(x, y)
        mobileInput.moveX = x
        mobileInput.moveY = y
    end)

    setupStick(self.rightBase, self.rightKnob, function(x, y)
        mobileInput.lookX = x
        mobileInput.lookY = y
    end)

    -- ============ КНОПКИ ВВЕРХ/ВНИЗ/ВЗРЫВ ============
    local function setupButton(btn, onDown, onUp)
    ...
    setupButton(self.upBtn, function() mobileInput.up = true end, function() mobileInput.up = false end)
    setupButton(self.downBtn, function() mobileInput.down = true end, function() mobileInput.down = false end)
    setupButton(self.boomBtn, function()
        mobileInput.boom = true
        task.delay(0.1, function() mobileInput.boom = false end)
    end, nil)
...
function MobileDroneControls:SetVisible(v)
    self.visible = v
    self.gui.Enabled = v
    if not v then
        mobileInput.moveX = 0
...
function MobileDroneControls:SetScale(s)
    Settings.DroneMobileScale = s
    -- упрощённо: пересоздать кнопки
    if self.gui then
        local px = function(n) return math.floor(n * s) end
        self.leftBase.Size = UDim2.fromOffset(px(140), px(140))
        self.leftBase.Position = UDim2.new(0, px(30), 1, -px(170))
        self.leftKnob.Size = UDim2.fromOffset(px(60), px(60))
        self.rightBase.Size = UDim2.fromOffset(px(140), px(140))
        self.rightBase.Position = UDim2.new(1, -px(170), 1, -px(170))
        self.rightKnob.Size = UDim2.fromOffset(px(60), px(60))
        self.upBtn.Size = UDim2.fromOffset(px(70), px(70))
        self.downBtn.Size = UDim2.fromOffset(px(70), px(70))
        self.boomBtn.Size = UDim2.fromOffset(px(70), px(70))
    end
end

-- ==========================================================
--  Drone flight control (CFrame-based, no physics)
-- ==========================================================
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    if Drone.destroyed then
        ...
        Drone.droneCF = Drone.droneCF * CFrame.new(0, -30 * dt, 0)
        if Drone.model then
            setDroneCFrame(Drone.model, Drone.base, Drone.droneCF)
        end
        if Drone.deathTimer <= 0 then
            Drone:Stop()
        end
        return
    end
    ...
    -- объединяем клавиатуру + мобильные контролы
    local moveX, moveY = 0, 0

    if UserInputService:IsKeyDown(Enum.KeyCode.W) then moveY = moveY - 1 end
    if UserInputService:IsKeyDown(Enum.KeyCode.S) then moveY = moveY + 1 end
    ...
    if Settings.DroneMobileControls then
        moveX = moveX + mobileInput.moveX
        moveY = moveY + mobileInput.moveY
    end
    ...
    if Settings.DroneMobileControls then
        if mobileInput.up then vert = vert + 1 end
        if mobileInput.down then vert = vert - 1 end
    end

    -- направление движения от камеры
    local moveDir = Vector3.new(0,0,0)
    if math.abs(moveX) > 0.05 or math.abs(moveY) > 0.05 then
        moveDir = (camCF.LookVector * -moveY) + (camCF.RightVector * moveX)
    end
    if vert ~= 0 then
        moveDir = moveDir + Vector3.new(0, vert, 0) * 0.6
    end
    ...
    if moveDir.Magnitude > 0.1 then
        targetVel = moveDir * Settings.DroneSpeed
    end
    ...
    if Drone.currentVel.Magnitude > 0.1 then
        ...
        if hit then
            Drone.currentVel = Vector3.new(0,0,0)
        else
    ...
    Drone.droneCF = Drone.droneCF:Lerp(targetCF, 0.25)
    ...
    if Settings.DroneCamera == "FPV" then
        ...
        local shakeAmp = Settings.DroneFPVShake and (speedNorm * 0.05) or 0
        ...
        if Settings.DroneMobileControls then strafe = strafe + mobileInput.moveX end
        local targetRoll = math.rad(strafe * 4)
        Drone.fpvRoll = Drone.fpvRoll + (targetRoll - Drone.fpvRoll) * 0.15

        local camBaseCF = Drone.droneCF * CFrame.new(0, 0.35, -0.4)
        Camera.CFrame = camBaseCF * CFrame.Angles(
            Drone.fpvShakeOff.Y,
            Drone.fpvShakeOff.X,
            Drone.fpvRoll
        )
        ...
        -- направление камеры можно крутить правым стиком
        local yaw = 0
        local pitch = 0
        if Settings.DroneMobileControls then
        ...
        local orbitCF = CFrame.new(Drone.droneCF.Position)
            * CFrame.Angles(0, yaw, 0)
            * CFrame.Angles(pitch, 0, 0)
            * CFrame.new(0, camOffset * 0.4, camOffset)

        Camera.CFrame = CFrame.lookAt(orbitCF.Position, Drone.droneCF.Position)
    end
...
trackConn(RunService.Heartbeat:Connect(function(dt)
    ...
    dt = math.clamp(dt, 0, 0.1)
    propellerAngle = propellerAngle + math.rad(2500) * dt
    ...
    for _, p in ipairs(Drone.props) do
        ...
        if p.blur and p.blur.Parent then
            ...
            p.blur.Transparency = 0.75 + math.sin(tick() * 30) * 0.05
        end
    end
end))
...
-- упрощённая HUD-модель (просто чтобы был вид)
local hudModel = Instance.new("Model")
hudModel.Parent = viewport

local hudCore = Instance.new("Part")
hudCore.Name = "Core"
hudCore.Size = Vector3.new(1.6, 0.55, 2.2)
hudCore.Color = Color3.fromRGB(35, 38, 45)
hudCore.Material = Enum.Material.SmoothPlastic
hudCore.Anchored = true
hudCore.CanCollide = false
hudCore.Parent = hudModel

local hudStripe = Instance.new("Part")
hudStripe.Size = Vector3.new(1.62, 0.06, 0.14)
hudStripe.Color = Color3.fromRGB(255, 40, 90)
hudStripe.Material = Enum.Material.Neon
hudStripe.Anchored = true
hudStripe.CanCollide = false
hudStripe.CFrame = hudCore.CFrame * CFrame.new(0, 0.28, 0)
hudStripe.Parent = hudModel
...
for i = 1, 4 do
    ...
    blade.Size = Vector3.new(1.6, 0.03, 0.14)
    blade.Color = Color3.fromRGB(230, 232, 240)
    blade.Material = Enum.Material.SmoothPlastic
    blade.Anchored = true
    blade.CanCollide = false
    blade.CFrame = hudCore.CFrame * CFrame.new(off)
    blade.Parent = hudModel
    table.insert(hudBlades, {blade = blade, baseCF = hudCore.CFrame * CFrame.new(off)})
end
...
local hudHit = Instance.new("TextLabel")
hudHit.BackgroundTransparency = 1
hudHit.Position = UDim2.fromOffset(105, 68)
hudHit.Size = UDim2.fromOffset(110, 16)
hudHit.Font = Enum.Font.GothamBold
hudHit.TextSize = 13
hudHit.TextColor3 = Color3.fromRGB(255, 180, 40)
hudHit.TextXAlignment = Enum.TextXAlignment.Left
hudHit.Text = "Target: none"
hudHit.Parent = droneHUD
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    for _, b in ipairs(hudBlades) do
        if b.blade then
            b.blade.CFrame = b.baseCF * CFrame.Angles(0, math.rad(720) * t, 0)
        end
    end
    ...
    if Drone.hitTimer > 0 then
        hudHit.Text = "Target: " .. Drone.lastHit
        hudHit.TextColor3 = Color3.fromRGB(255, 100, 100)
    else
        hudHit.Text = "Target: none"
        hudHit.TextColor3 = Color3.fromRGB(255, 180, 40)
    end
...
TabDrone:Section({ Title = "Mobile Controls" })
TabDrone:Toggle({
    Title = "Show mobile joysticks & buttons",
    Value = true,
    Callback = function(v)
        Settings.DroneMobileControls = v
        if Drone.MobileControls then
            Drone.MobileControls:SetVisible(v and Drone.active or false)
        end
    end,
})
TabDrone:Slider({
    Title = "Mobile controls scale",
    Value = { Min = 0.6, Max = 1.6, Default = 1.0 },
    Callback = function(v)
        Settings.DroneMobileScale = v
        if Drone.MobileControls then
            Drone.MobileControls:SetScale(v)
        end
    end,
})
...
getgenv().MellHackUnload = function()
    ...
    if Drone.MobileControls then
        pcall(function() Drone.MobileControls:SetVisible(false) end)
    end
...
-- ==========================================================
--  Создание мобильных контролов + подключение к Drone:Start/Stop
-- ==========================================================
Drone.MobileControls = MobileDroneControls.new(ESPGui)

local _origDroneStart = Drone.Start
function Drone:Start()
    _origDroneStart(self)
    if self.active and Settings.DroneMobileControls then
        self.MobileControls:SetVisible(true)
    end
end

local _origDroneStop = Drone.Stop
function Drone:Stop()
    _origDroneStop(self)
    if self.MobileControls then
        self.MobileControls:SetVisible(false)
    end
end
