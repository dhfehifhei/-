-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v5
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local payload = {
        __meta = {
            name = name, created = os.time(),
            author = LocalPlayer.Name, placeId = game.PlaceId, version = "5.0",
        },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "5.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "5.0" }, settings = data }
    end
...
local function buildDroneModel()
    ...
    core.Name = "Core"
    core.Size = Vector3.new(2, 0.6, 2)
    core.Shape = Enum.PartType.Block
    core.Color = Color3.fromRGB(30, 30, 35)
    ...
    dome.Massless = true
    dome.Parent = model
    local domeWeld = Instance.new("WeldConstraint")
    domeWeld.Part0 = core
    domeWeld.Part1 = dome
    domeWeld.Parent = dome
    dome.CFrame = core.CFrame * CFrame.new(0, 0.4, 0)

    local led = Instance.new("Part")
    led.Name = "LED"
    led.Shape = Enum.PartType.Ball
    led.Size = Vector3.new(0.25, 0.25, 0.25)
    led.Material = Enum.Material.Neon
    led.Color = Color3.fromRGB(255, 40, 90)
    ...
    ledWeld.Part1 = led
    ledWeld.Parent = led
    led.CFrame = core.CFrame * CFrame.new(0, 0.1, -1.05)

    local propOffsets = {
        Vector3.new( 1.1, 0.15,  1.1),
        Vector3.new(-1.1, 0.15,  1.1),
        Vector3.new( 1.1, 0.15, -1.1),
        Vector3.new(-1.1, 0.15, -1.1),
    }
    for i, off in ipairs(propOffsets) do
        ...
        arm.Name = "Arm" .. i
        arm.Size = Vector3.new(0.15, 0.15, 1.3)
        arm.Material = Enum.Material.Metal
        arm.Color = Color3.fromRGB(60, 60, 70)
        ...
        armWeld.Part1 = arm
        armWeld.Parent = arm
        arm.CFrame = core.CFrame * CFrame.new(off.X/2, off.Y, off.Z/2) * CFrame.Angles(0, math.rad(i*90), 0)

        local motor = Instance.new("Part")
        motor.Name = "Motor" .. i
        motor.Shape = Enum.PartType.Cylinder
        motor.Size = Vector3.new(0.3, 0.3, 0.3)
        motor.Material = Enum.Material.Metal
        motor.Color = Color3.fromRGB(40, 40, 50)
        motor.CanCollide = false
        motor.Massless = true
        motor.Parent = model
        motor.CFrame = core.CFrame * CFrame.new(off)

        local blade = Instance.new("Part")
        blade.Name = "Blade" .. i
        blade.Size = Vector3.new(1.4, 0.05, 0.15)
        ...
        blade.Massless = true
        blade.Parent = model
        blade.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.05, 0))
        ...
        table.insert(Drone.props, {blade = blade, baseCF = core.CFrame * CFrame.new(off), idx = i})
    end
...
function Drone:Start()
    ...
    self.base.CFrame = hrp.CFrame * CFrame.new(0, 2.5, 0)
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    local speed = Settings.DroneSpeed
    local targetVel = moveDir.Magnitude > 0 and moveDir.Unit * speed or Vector3.new(0,0,0)

    if Drone.velocity then
        Drone.velocity.VectorVelocity = targetVel
    end

    if Drone.align then
        local lookCF
        if moveDir.Magnitude > 0 then
            lookCF = CFrame.lookAt(base.Position, base.Position + moveDir.Unit)
        else
            lookCF = CFrame.lookAt(base.Position, base.Position + camCF.LookVector)
        end
        Drone.align.CFrame = lookCF
    end

    if Settings.DroneCamera == "FPV" then
        Camera.CFrame = base.CFrame * CFrame.new(0, 0.4, -0.5)
    else
        local camOffset = Settings.DroneCameraBack
        Camera.CFrame = CFrame.lookAt(base.Position + base.CFrame.LookVector * -camOffset + Vector3.new(0, 4, 0), base.Position)
    end
end))
...
trackConn(RunService.Heartbeat:Connect(function(dt)
    ...
    for _, p in ipairs(Drone.props) do
        if p.blade and p.blade.Parent then
            p.blade.CFrame = p.blade.CFrame * CFrame.Angles(0, math.rad(40) * dt * 60, 0)
        end
    end
end))
...
hudCore.Name = "Core"
hudCore.Size = Vector3.new(2, 0.6, 2)
hudCore.Color = Color3.fromRGB(30, 30, 35)
...
for i = 1, 4 do
    ...
    local blade = Instance.new("Part")
    blade.Size = Vector3.new(1.3, 0.05, 0.15)
    blade.Color = Color3.fromRGB(220, 220, 220)
    blade.Anchored = true
    blade.CanCollide = false
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    for _, b in ipairs(hudBlades) do
        if b.blade then
            b.blade.CFrame = b.baseCF * CFrame.Angles(0, math.rad(360) * t, 0)
        end
    end

    hudSpeed.Text = "Speed: " .. tostring(Settings.DroneSpeed)
...
TabDrone:Dropdown({
    ...
    Values = { "FPV", "TPV" },
    Value = "TPV",
    Callback = function(v) Settings.DroneCamera = v end,
})
TabDrone:Dropdown({
    Title = "HUD Corner",
    Values = { "TL", "TR", "BL", "BR" },
...
trackConn(RunService.RenderStepped:Connect(function()
    ...
    -- Skip humanoid movement mods while drone is active
    if Drone.active then
        -- still allow visual / lighting / external mods below
    else
