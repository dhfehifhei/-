-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v12
-- ==========================================================
...
-- ==========================================================
--  User info panel (avatar + nickname) в меню
-- ==========================================================
do
...
local function buildDroneModel()
    ...
    for i, data in ipairs({
        { pos = Vector3.new( 0.5, -0.42,  0.6) },
        { pos = Vector3.new(-0.5, -0.42,  0.6) },
        { pos = Vector3.new( 0.5, -0.42, -0.4) },
        { pos = Vector3.new(-0.5, -0.42, -0.4) },
    }) do
...
-- ==========================================================
--  DRONE FLIGHT — дрон летит туда, куда смотрит камера
-- ==========================================================
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    if isMobile then
        ...
        if Settings.DroneCamera == "FPV" then
            local camPos = Drone.droneCF * CFrame.new(0, 0.35, -0.5).Position
            ...
            local camPos = Drone.droneCF.Position + offset.Position + Vector3.new(0, camOffset * 0.35, 0)
            camCF = CFrame.lookAt(camPos, Drone.droneCF.Position)
        end
    else
        if Settings.DroneCamera == "FPV" then
            camCF = Drone.droneCF * CFrame.new(0, 0.35, -0.5)
        else
            camCF = Camera.CFrame
        end
    end
    ...
    if moveInput.Magnitude > 0.05 then
        local forward = Vector3.new(camLook.X, camLook.Y, camLook.Z)
        ...
        if isMobile then
            local pitchStrength = math.clamp(math.abs(camLook.Y), 0, 1)
            if pitchStrength > 0.15 then
                moveDir = camLook.Unit * (pitchStrength * 0.6)
            end
        end
    end
