-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v9
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "9.0" },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "9.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "9.0" }, settings = data }
    end
...
-- ==========================================================
--  DRONE
--  Модель в стиле DJI Mavic + управление через
--  стандартный Roblox ControlModule (джойстики и WASD).
-- ==========================================================
local Drone = {
    model        = nil,
    base         = nil,
...
local function buildDroneModel()
    ...
    for i, pos in ipairs({
        ...
        leg.Massless = true
        leg.Parent = model
        leg.CFrame = core.CFrame * CFrame.new(pos)
        ...
        foot.Massless = true
        foot.Parent = model
        foot.CFrame = core.CFrame * CFrame.new(pos.X, pos.Y - 0.25, pos.Z)
    end

    -- Лучи + моторы + лопасти
    for i, data in ipairs({
        { off = Vector3.new( 1.4, 0.05,  1.4) },
        { off = Vector3.new(-1.4, 0.05,  1.4) },
        { off = Vector3.new( 1.4, 0.05, -1.4) },
        { off = Vector3.new(-1.4, 0.05, -1.4) },
    }) do
...
function Drone:Start()
    ...
    -- Даём Roblox управлять камерой (нужно для свайпа на мобилке)
    task.wait(0.05)
    pcall(function()
    ...
    self.active = true
    WindUI:Notify({ Title = "Drone", Content = "Drone launched! Use Roblox joystick to fly.", Icon = "plane", Duration = 4 })
end
...
--  • Левый джойстик (моб) / WASD (ПК) → движение
--  • Свайп (моб) / мышь (ПК) → поворот камеры
--  • Прыжок (моб) / Space (ПК) → вверх
--  • LeftCtrl (ПК) → вниз
-- ==========================================================
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    if moveInput.Magnitude > 0.05 then
        ...
        -- Roblox: W = -Z на джойстике; переводим в forward/right
    ...
    -- в FPV наклон камеры вверх/вниз тоже влияет на вертикаль
    if Settings.DroneCamera == "FPV" then
        vert = vert + math.clamp(camCF.LookVector.Y, -1, 1) * 0.5
    end
    ...
    if Settings.DroneCamera == "FPV" then
        ...
        -- TPV — используем камеру, которой Roblox даёт крутить свайпом
        Camera.CameraType = Enum.CameraType.Scriptable
        Camera.FieldOfView = Camera.FieldOfView + (90 - Camera.FieldOfView) * 0.15

        local camOffset = Settings.DroneCameraBack
        local behind = camCF.LookVector * -camOffset + Vector3.new(0, camOffset * 0.35, 0)
        local desired = CFrame.lookAt(Drone.droneCF.Position + behind, Drone.droneCF.Position)
