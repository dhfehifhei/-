-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v2
-- ==========================================================

local function safeLog(...)
    local args = {...}
    pcall(function() warn("[MellHack]", table.unpack(args)) end)
end
...
if not WindUI then
    ...
    for _, url in ipairs({
        ...
        src = httpGet(url)
        if src then safeLog("WindUI: скачал с " .. url); break end
    end
    if not src then
        safeLog("WindUI не скачался")
        StarterGui:SetCore("SendNotification", {
            Title = "MellHack", Text = "WindUI не скачался. Проверь интернет.", Duration = 8,
        })
        return
    end
...
-- ==========================================================
--  Custom ScreenGui for ESP / Crosshair / Watermark / Bullet tracers
-- ==========================================================
local gethuiFn = gethui or function()
    return (game:GetService("RunService"):IsStudio() and LocalPlayer:WaitForChild("PlayerGui")) or CoreGui
end
local ParentGui
...
-- ==========================================================
--  ESP через BillboardGui (работает на Delta)
-- ==========================================================
local ESPCache = {}
local function ensureESP(player)
    ...
    data.Visible = { box = false, name = false, hp = false }
...
-- ==========================================================
--  Tracer GUI (lines from bottom center → target)
-- ==========================================================
local TracerCache = {}
...
-- ==========================================================
--  Skeleton ESP (bones: head-chest, chest-hip, hip-legs, arms)
-- ==========================================================
local SkeletonCache = {}
...
watermarkLabel.Size = UDim2.fromOffset(300, 24)
...
trackConn(RunService.RenderStepped:Connect(function()
    ...
    if Settings.Watermark then
        local server = game:GetService("Players"):GetPlayers()
        watermarkLabel.Text = string.format("MellHack | %s | %d FPS | %d players | ping %d ms",
...
-- Подписка на выстрел: если Tool активируется — рисуем трейсер к цели
local function onToolActivated()
    ...
    -- выбираем ближайшего противника по экрану
    local targetPos = nil
    local minDst = 9999
    ...
    if not targetPos then
        -- если нет цели — линия к точке взгляда
...
local function saveConfig()
    if not writefile then
        WindUI:Notify({ Title = "MellHack", Content = "writefile недоступен", Icon = "x", Duration = 4 })
        return
    end
    ...
    if ok then
        ...
        WindUI:Notify({ Title = "MellHack", Content = "Конфиг сохранён", Icon = "check", Duration = 3 })
    end
end
...
local function loadConfig()
    ...
    if not isfile(CONFIG_FILE) then
        WindUI:Notify({ Title = "MellHack", Content = "Конфиг не найден", Icon = "x", Duration = 3 })
        return
    end
    ...
    if not ok or not data then
        WindUI:Notify({ Title = "MellHack", Content = "Не смог прочитать", Icon = "x", Duration = 3 })
        return
    end
    ...
    WindUI:Notify({ Title = "MellHack", Content = "Конфиг загружен", Icon = "check", Duration = 3 })
end
...
local function resetConfig()
    ...
    ApplyMenuColorsToTheme()
    WindUI:Notify({ Title = "MellHack", Content = "Настройки сброшены", Icon = "check", Duration = 3 })
end
...
local function notifyState(name, enabled)
    WindUI:Notify({
        Title = "MellHack",
        Content = name .. (enabled and " — ВКЛ" or " — ВЫКЛ"),
...
TabMovement:Toggle({ Title = "Fly / Полет", Value = false, Callback = function(v)
...
TabCombat:Slider({ Title = "Trigger Delay (сек)", Value = { Min = 0.0, Max = 1, Default = 0.05 }, Callback = function(v) Settings.TriggerbotDelay = v end })
...
TabColors:Colorpicker({ Title = "Accent / Акцент", Default = Settings.ColorMenu, Transparency = Settings.AlphaMenu,
...
--  EXTRAS
-- ==========================================================
TabExtras:Section({ Title = "Config / Конфиг" })
...
TabExtras:Section({ Title = "Teleport / Телепорт" })
TabExtras:Dropdown({
    ...
    Callback = function()
        task.spawn(function()
            pcall(function()
                ...
                if not body then WindUI:Notify({ Title = "MellHack", Content = "HTTP недоступен.", Icon = "x" }); return end
                ...
                if #servers > 0 then
                    ...
                    WindUI:Notify({ Title = "MellHack", Content = "Серверов нет.", Icon = "x" })
                end
            end)
        end)
    end })
...
-- ==========================================================
--  FOV CIRCLE (через Frame + UICorner)
-- ==========================================================
...
trackConn(RunService.RenderStepped:Connect(function()
    ...
    elseif Settings.AntiAim then
        ...
        elseif mode == "Jitter" then
            local j = math.rad(math.random(-Settings.JitterRange, Settings.JitterRange))
            finalAngle = yawOffsetRad + j
