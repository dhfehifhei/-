-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v6
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "6.0" },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "6.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "6.0" }, settings = data }
    end
...
local function buildDroneModel()
    ...
    -- Core body (main physics block)
    local core = Instance.new("Part")
    core.Name = "Core"
    core.Size = Vector3.new(2.2, 0.5, 2.2)
    core.Shape = Enum.PartType.Block
    core.Color = Color3.fromRGB(25, 25, 32)
    ...
    core.CustomPhysicalProperties = PhysicalProperties.new(3, 0.5, 0.5, 1, 1)
    core.Parent = model
    ...
    topPlate.Size = Vector3.new(1.6, 0.1, 1.6)
    topPlate.Material = Enum.Material.Neon
    topPlate.Color = Color3.fromRGB(255, 40, 90)
    topPlate.CanCollide = false
    topPlate.Massless = true
    topPlate.Parent = model
    topPlate.CFrame = core.CFrame * CFrame.new(0, 0.3, 0)
    local tpw = Instance.new("WeldConstraint")
    tpw.Part0 = core; tpw.Part1 = topPlate; tpw.Parent = topPlate

    -- Glass dome (FPV view pod)
    local dome = Instance.new("Part")
    dome.Name = "Dome"
    dome.Shape = Enum.PartType.Ball
    dome.Size = Vector3.new(1.1, 1.1, 1.1)
    ...
    dome.Massless = true
    dome.Parent = model
    dome.CFrame = core.CFrame * CFrame.new(0, 0.45, 0)
    local dw = Instance.new("WeldConstraint")
    dw.Part0 = core; dw.Part1 = dome; dw.Parent = dome

    -- Front camera lens
    local lens = Instance.new("Part")
    lens.Name = "Lens"
    lens.Shape = Enum.PartType.Cylinder
    lens.Size = Vector3.new(0.05, 0.4, 0.4)
    lens.Material = Enum.Material.Neon
    lens.Color = Color3.fromRGB(40, 40, 60)
    lens.CanCollide = false
    lens.Massless = true
    lens.Parent = model
    lens.CFrame = core.CFrame * CFrame.new(0, 0.45, -0.55) * CFrame.Angles(0, 0, math.rad(90))
    local lw = Instance.new("WeldConstraint")
    lw.Part0 = core; lw.Part1 = lens; lw.Parent = lens

    -- Front LED (red)
    local frontLed = Instance.new("Part")
    frontLed.Name = "FrontLED"
    frontLed.Shape = Enum.PartType.Ball
    frontLed.Size = Vector3.new(0.22, 0.22, 0.22)
    frontLed.Material = Enum.Material.Neon
    frontLed.Color = Color3.fromRGB(255, 40, 90)
    frontLed.CanCollide = false
    frontLed.Massless = true
    frontLed.Parent = model
    frontLed.CFrame = core.CFrame * CFrame.new(0, 0.05, -1.15)
    local flw = Instance.new("WeldConstraint")
    flw.Part0 = core; flw.Part1 = frontLed; flw.Parent = frontLed

    -- Back LED (green)
    local backLed = Instance.new("Part")
    backLed.Name = "BackLED"
    backLed.Shape = Enum.PartType.Ball
    backLed.Size = Vector3.new(0.18, 0.18, 0.18)
    backLed.Material = Enum.Material.Neon
    backLed.Color = Color3.fromRGB(0, 255, 120)
    backLed.CanCollide = false
    backLed.Massless = true
    backLed.Parent = model
    backLed.CFrame = core.CFrame * CFrame.new(0, 0.05, 1.15)
    local blw = Instance.new("WeldConstraint")
    blw.Part0 = core; blw.Part1 = backLed; blw.Parent = backLed

    -- Antenna
    local antenna = Instance.new("Part")
    antenna.Name = "Antenna"
    antenna.Size = Vector3.new(0.05, 0.6, 0.05)
    ...
    antenna.CFrame = core.CFrame * CFrame.new(0, 0.85, 0.6)
    local anw = Instance.new("WeldConstraint")
    anw.Part0 = core; anw.Part1 = antenna; anw.Parent = antenna

    local antennaBall = Instance.new("Part")
    antennaBall.Name = "AntennaBall"
    antennaBall.Shape = Enum.PartType.Ball
    antennaBall.Size = Vector3.new(0.15, 0.15, 0.15)
    antennaBall.Material = Enum.Material.Neon
    antennaBall.Color = Color3.fromRGB(255, 180, 40)
    antennaBall.CanCollide = false
    antennaBall.Massless = true
    antennaBall.Parent = model
    antennaBall.CFrame = core.CFrame * CFrame.new(0, 1.2, 0.6)
    local abw = Instance.new("WeldConstraint")
    abw.Part0 = core; abw.Part1 = antennaBall; abw.Parent = antennaBall

    -- 4 arms with motors + propellers
    local propOffsets = {
        Vector3.new( 1.15, 0.1,  1.15),
        Vector3.new(-1.15, 0.1,  1.15),
        Vector3.new( 1.15, 0.1, -1.15),
        Vector3.new(-1.15, 0.1, -1.15),
    }

    for i, off in ipairs(propOffsets) do
        -- Arm (diagonal bar)
        local arm = Instance.new("Part")
        arm.Name = "Arm" .. i
        arm.Size = Vector3.new(0.18, 0.18, 1.7)
        arm.Material = Enum.Material.Metal
        arm.Color = Color3.fromRGB(45, 45, 55)
        arm.CanCollide = false
        arm.Massless = true
        arm.Parent = model

        local angle = math.atan2(off.X, off.Z)
        arm.CFrame = core.CFrame * CFrame.new(off.X/2, off.Y, off.Z/2) * CFrame.Angles(0, angle, 0)
        local aw = Instance.new("WeldConstraint")
        aw.Part0 = core; aw.Part1 = arm; aw.Parent = arm

        -- Motor
        local motor = Instance.new("Part")
        motor.Name = "Motor" .. i
        motor.Shape = Enum.PartType.Cylinder
        motor.Size = Vector3.new(0.35, 0.35, 0.35)
        motor.Material = Enum.Material.Metal
        motor.Color = Color3.fromRGB(30, 30, 40)
        ...
        mw.Part0 = core; mw.Part1 = motor; mw.Parent = motor

        -- Motor cap (neon accent)
        local cap = Instance.new("Part")
        cap.Name = "MotorCap" .. i
        cap.Shape = Enum.PartType.Cylinder
        cap.Size = Vector3.new(0.1, 0.4, 0.4)
        cap.Material = Enum.Material.Neon
        cap.Color = (i % 2 == 0) and Color3.fromRGB(255, 40, 90) or Color3.fromRGB(80, 200, 255)
        cap.CanCollide = false
        cap.Massless = true
        cap.Parent = model
        cap.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.2, 0)) * CFrame.Angles(0, 0, math.rad(90))
        local cw = Instance.new("WeldConstraint")
        cw.Part0 = core; cw.Part1 = cap; cw.Parent = cap

        -- Blade group (2 blades = X shape)
        local bladeGroup = Instance.new("Model")
        bladeGroup.Name = "Blades" .. i
        bladeGroup.Parent = model

        local blade1 = Instance.new("Part")
        blade1.Name = "Blade1"
        blade1.Size = Vector3.new(1.6, 0.04, 0.14)
        blade1.Material = Enum.Material.Plastic
        blade1.Color = Color3.fromRGB(220, 220, 230)
        blade1.Transparency = 0.15
        blade1.CanCollide = false
        blade1.Massless = true
        blade1.Parent = bladeGroup
        blade1.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.28, 0))
        local b1w = Instance.new("WeldConstraint")
        b1w.Part0 = core; b1w.Part1 = blade1; b1w.Parent = blade1

        local blade2 = Instance.new("Part")
        blade2.Name = "Blade2"
        blade2.Size = Vector3.new(0.14, 0.04, 1.6)
        blade2.Material = Enum.Material.Plastic
        blade2.Color = Color3.fromRGB(220, 220, 230)
        blade2.Transparency = 0.15
        blade2.CanCollide = false
        blade2.Massless = true
        blade2.Parent = bladeGroup
        blade2.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.28, 0))
        local b2w = Instance.new("WeldConstraint")
        b2w.Part0 = core; b2w.Part1 = blade2; b2w.Parent = blade2

        -- Blade blur (fake motion blur ring)
        local blur = Instance.new("Part")
        blur.Name = "BladeBlur" .. i
        blur.Shape = Enum.PartType.Cylinder
        blur.Size = Vector3.new(0.02, 1.5, 1.5)
        ...
        blur.Massless = true
        blur.Parent = model
        blur.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.3, 0)) * CFrame.Angles(0, 0, math.rad(90))
        local blw2 = Instance.new("WeldConstraint")
        blw2.Part0 = core; blw2.Part1 = blur; blw2.Parent = blur

        table.insert(Drone.props, {
            blades = {blade1, blade2},
            blur = blur,
            motor = motor,
            baseCF = core.CFrame * CFrame.new(off + Vector3.new(0, 0.28, 0)),
        })
    end
...
function Drone:Start()
    ...
    self.base.CFrame = hrp.CFrame * CFrame.new(0, 3, 0)
    self.base.AssemblyLinearVelocity = Vector3.new(0,0,0)
    self.base.AssemblyAngularVelocity = Vector3.new(0,0,0)
    self.base.CustomPhysicalProperties = PhysicalProperties.new(3, 0.5, 0.5, 1, 1)

    -- Attachment for constraints
    local attach = Instance.new("Attachment")
    attach.Parent = self.base

    -- Anti-gravity (cancel gravity so it hovers)
    local vf = Instance.new("VectorForce")
    vf.Attachment0 = attach
    vf.RelativeTo = Enum.ActuatorRelativeTo.World
    vf.ApplyAtCenterOfMass = true
    vf.Force = Vector3.new(0, self.base:GetMass() * Workspace.Gravity, 0)
    vf.Parent = self.base
    self.antiGravity = vf

    -- AlignPosition: target position, drone smoothly moves toward it
    local ap = Instance.new("AlignPosition")
    ap.Attachment0 = attach
    ap.Mode = Enum.PositionAlignmentMode.OneAttachment
    ap.Position = self.base.Position
    ap.MaxForce = self.base:GetMass() * 2000
    ap.Responsiveness = 35
    ap.ApplyAtCenterOfMass = true
    ap.Parent = self.base
    self.alignPos = ap

    -- AlignOrientation: keeps drone level, only yaw changes
    local ao = Instance.new("AlignOrientation")
    ao.Attachment0 = attach
    ao.Mode = Enum.OrientationAlignmentMode.OneAttachment
    ao.MaxTorque = self.base:GetMass() * 2000
    ao.Responsiveness = 25
    ao.Parent = self.base
    self.align = ao

    -- initial CFrame orientation
    ao.CFrame = CFrame.lookAt(self.base.Position, self.base.Position + Vector3.new(0,0,-1))
...
function Drone:Stop()
    ...
    -- cleanup FPV effects
    if Drone.fpvVignette then
        pcall(function() Drone.fpvVignette:Destroy() end)
        Drone.fpvVignette = nil
    end
...
-- ==========================================================
--  DRONE FPV overlay (vignette, crosshair)
-- ==========================================================
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    if Drone.destroyed then
        ...
        if Drone.base then
            Drone.base.AssemblyLinearVelocity = Vector3.new(0, -50, 0)
        end
    ...
    if moveDir.Magnitude > 0 then
        targetVel = moveDir.Unit * Settings.DroneSpeed
    else
    ...
    if Drone.alignPos then
        Drone.alignPos.Position = Drone.alignPos.Position + Drone.currentVel * dt
    end
    ...
    if Drone.align then
        ...
        if Drone.currentVel.Magnitude > 5 then
            lookTarget = Drone.currentVel.Unit
        else
        ...
        Drone.align.CFrame = CFrame.lookAt(base.Position, base.Position + lookTarget)
    end
    ...
    if Settings.DroneCamera == "FPV" then
        ...
        -- camera tilt when strafing (roll)
        local strafe = 0
        ...
        local camBaseCF = base.CFrame * CFrame.new(0, 0.35, -0.4)
        ...
        local behind = (base.CFrame.LookVector * -camOffset) + Vector3.new(0, camOffset * 0.4, 0)
        local desired = CFrame.lookAt(base.Position + behind, base.Position)
        Camera.CFrame = Camera.CFrame:Lerp(desired, 0.2)
    end

    -- FPV overlay visibility
...
trackConn(RunService.Heartbeat:Connect(function(dt)
    ...
    for _, p in ipairs(Drone.props) do
        if p.blades then
            for _, blade in ipairs(p.blades) do
                if blade and blade.Parent then
                    blade.CFrame = blade.CFrame * CFrame.Angles(0, math.rad(3600) * dt, 0)
                end
            end
        end
...
local function setHUDCorner()
    ...
    elseif c == "BL" then
        droneHUD.Position = UDim2.new(0, 20, 1, -20)
        droneHUD.AnchorPoint = Vector2.new(0, 1)
    else
        droneHUD.Position = UDim2.new(1, -20, 1, -20)
...
-- simplified mini-model for HUD
local hudModel = Instance.new("Model")
hudModel.Parent = viewport

local hudCore = Instance.new("Part")
hudCore.Name = "Core"
hudCore.Size = Vector3.new(2.2, 0.5, 2.2)
hudCore.Color = Color3.fromRGB(25, 25, 32)
hudCore.Material = Enum.Material.Metal
hudCore.Anchored = true
hudCore.CanCollide = false
hudCore.Parent = hudModel

local hudPlate = Instance.new("Part")
hudPlate.Size = Vector3.new(1.6, 0.1, 1.6)
hudPlate.Color = Color3.fromRGB(255, 40, 90)
hudPlate.Material = Enum.Material.Neon
hudPlate.Anchored = true
hudPlate.CanCollide = false
hudPlate.CFrame = hudCore.CFrame * CFrame.new(0, 0.3, 0)
hudPlate.Parent = hudModel

local hudDome = Instance.new("Part")
hudDome.Shape = Enum.PartType.Ball
hudDome.Size = Vector3.new(1.1, 1.1, 1.1)
hudDome.Color = Color3.fromRGB(80, 200, 255)
hudDome.Material = Enum.Material.Glass
hudDome.Transparency = 0.4
hudDome.Anchored = true
hudDome.CanCollide = false
hudDome.CFrame = hudCore.CFrame * CFrame.new(0, 0.45, 0)
hudDome.Parent = hudModel

local hudBlades = {}
for i = 1, 4 do
    local off = ({
        Vector3.new( 1.15, 0.4,  1.15),
        Vector3.new(-1.15, 0.4,  1.15),
        Vector3.new( 1.15, 0.4, -1.15),
        Vector3.new(-1.15, 0.4, -1.15),
    })[i]
    local blade = Instance.new("Part")
    blade.Size = Vector3.new(1.5, 0.04, 0.14)
