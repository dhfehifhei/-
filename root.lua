-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v10
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "10.0" },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "10.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "10.0" }, settings = data }
    end
...
-- ==========================================================
--  DRONE
--  Реалистичная модель + звук + мобильная камера
-- ==========================================================
local Drone = {
    model        = nil,
    base         = nil,
    ...
    fpvRoll      = 0,
    -- Камера (для мобильного управления)
    camYaw       = 0,
    camPitch     = -0.15,
    -- Звук
    soundEngine  = nil,
...
-- ============ ЗВУКИ ============
local SOUND_ENGINE  = "rbxassetid://9120386436"  -- drone motor hum
local SOUND_STARTUP = "rbxassetid://9125402237"  -- power up
local SOUND_HIT     = "rbxassetid://9120386675"  -- impact/explosion
...
-- ============ МОДЕЛЬ ДРОНА (улучшенная) ============
local function buildDroneModel()
    local model = Instance.new("Model")
    model.Name = "MellDrone"

    local DARK   = Color3.fromRGB(28, 30, 36)
    local DARKER = Color3.fromRGB(15, 16, 20)
    local LIGHT  = Color3.fromRGB(200, 202, 210)
    local GRAY   = Color3.fromRGB(70, 74, 82)
    local ACCENT = Color3.fromRGB(255, 40, 90)

    -- ============ КОРПУС: две вытянутые капсулы ============
    -- Верхняя часть (спина) — вытянутая вперёд
    local topBody = Instance.new("Part")
    topBody.Name = "TopBody"
    topBody.Size = Vector3.new(1.4, 0.35, 2.6)
    topBody.Color = DARK
    topBody.Material = Enum.Material.SmoothPlastic
    topBody.Anchored = true
    topBody.CanCollide = false
    topBody.Massless = true
    topBody.Parent = model
    model.PrimaryPart = topBody

    -- закруглённый "нос" — сфера спереди
    local nose = Instance.new("Part")
    nose.Name = "Nose"
    nose.Shape = Enum.PartType.Ball
    nose.Size = Vector3.new(1.4, 0.35, 0.5)
    nose.Color = DARK
    ...
    nose.Massless = true
    nose.Parent = model
    nose.CFrame = topBody.CFrame * CFrame.new(0, 0, -1.3)

    -- нижняя часть (брюхо)
    local botBody = Instance.new("Part")
    botBody.Name = "BotBody"
    botBody.Size = Vector3.new(1.2, 0.2, 2.2)
    botBody.Color = DARKER
    botBody.Material = Enum.Material.SmoothPlastic
    botBody.Anchored = true
    botBody.CanCollide = false
    botBody.Massless = true
    botBody.Parent = model
    botBody.CFrame = topBody.CFrame * CFrame.new(0, -0.27, 0.1)

    -- верхняя тонкая крышка
    local topCover = Instance.new("Part")
    topCover.Name = "TopCover"
    topCover.Size = Vector3.new(1.1, 0.06, 2.2)
    ...
    topCover.CFrame = topBody.CFrame * CFrame.new(0, 0.19, 0)

    -- акцентная полоса
    local stripe = Instance.new("Part")
    stripe.Name = "Stripe"
    stripe.Size = Vector3.new(1.42, 0.05, 0.14)
    stripe.Color = ACCENT
    ...
    stripe.Parent = model
    stripe.CFrame = topBody.CFrame * CFrame.new(0, 0.21, 0)

    -- маленькая красная точка-индикатор
    ...
    indicator.CFrame = topBody.CFrame * CFrame.new(0, 0.24, -0.6)

    -- ============ КАМЕРА-ПОДВЕС СНИЗУ ============
    local gimbalYoke = Instance.new("Part")
    gimbalYoke.Name = "GimbalYoke"
    gimbalYoke.Size = Vector3.new(0.4, 0.2, 0.4)
    ...
    gimbalYoke.CFrame = topBody.CFrame * CFrame.new(0, -0.45, -0.55)

    local camBall = Instance.new("Part")
    camBall.Name = "CameraBall"
    camBall.Shape = Enum.PartType.Ball
    camBall.Size = Vector3.new(0.5, 0.5, 0.5)
    ...
    camBall.CFrame = topBody.CFrame * CFrame.new(0, -0.6, -0.55)

    local camLens = Instance.new("Part")
    camLens.Name = "CameraLens"
    camLens.Shape = Enum.PartType.Cylinder
    camLens.Size = Vector3.new(0.06, 0.3, 0.3)
    camLens.Color = Color3.fromRGB(8, 10, 15)
    ...
    camLens.CFrame = topBody.CFrame * CFrame.new(0, -0.6, -0.8) * CFrame.Angles(0, 0, math.rad(90))

    -- ============ НОЖКИ (складные) ============
    -- по 2 с каждой стороны, спрятаны под брюхо
    for i, data in ipairs({
        { pos = Vector3.new( 0.5, -0.42,  0.6), flip = 1 },
        { pos = Vector3.new(-0.5, -0.42,  0.6), flip = -1 },
        { pos = Vector3.new( 0.5, -0.42, -0.4), flip = 1 },
        { pos = Vector3.new(-0.5, -0.42, -0.4), flip = -1 },
    }) do
        local leg = Instance.new("Part")
        leg.Name = "Leg" .. i
        leg.Size = Vector3.new(0.08, 0.35, 0.08)
        leg.Color = DARKER
        ...
        leg.Massless = true
        leg.Parent = model
        leg.CFrame = topBody.CFrame * CFrame.new(data.pos)

        local foot = Instance.new("Part")
        foot.Name = "Foot" .. i
        foot.Size = Vector3.new(0.12, 0.04, 0.28)
        foot.Color = DARKER
        ...
        foot.Massless = true
        foot.Parent = model
        foot.CFrame = topBody.CFrame * CFrame.new(data.pos.X, data.pos.Y - 0.18, data.pos.Z)
    end

    -- ============ 4 ЛУЧА + МОТОРЫ + ЛОПАСТИ ============
    for i, data in ipairs({
        { off = Vector3.new( 1.45, 0.05,  1.45) },
        { off = Vector3.new(-1.45, 0.05,  1.45) },
        { off = Vector3.new( 1.45, 0.05, -1.45) },
        { off = Vector3.new(-1.45, 0.05, -1.45) },
    }) do
        ...
        -- Луч (тонкая труба с наклоном)
        local arm = Instance.new("Part")
        arm.Name = "Arm" .. i
        arm.Size = Vector3.new(2.2, 0.14, 0.22)
        arm.Color = DARK
        ...
        wrap.Name = "Wrap" .. i
        wrap.Size = Vector3.new(0.28, 0.22, 0.36)
        wrap.Color = DARKER
        ...
        wrap.Massless = true
        wrap.Parent = model
        wrap.CFrame = topBody.CFrame
            * CFrame.new(off.X * 0.35, off.Y, off.Z * 0.35)
            * CFrame.Angles(0, armAngle, 0)
        ...
        motor.Size = Vector3.new(0.42, 0.48, 0.48)
        motor.Color = DARK
        ...
        motor.Massless = true
        motor.Parent = model
        motor.CFrame = topBody.CFrame * CFrame.new(off) * CFrame.Angles(0, 0, math.rad(90))

        -- Верхняя крышка мотора (неон, чередуется)
        local cap = Instance.new("Part")
        cap.Name = "MotorCap" .. i
        cap.Shape = Enum.PartType.Cylinder
        cap.Size = Vector3.new(0.08, 0.42, 0.42)
        ...
        cap.Massless = true
        cap.Parent = model
        cap.CFrame = topBody.CFrame * CFrame.new(off + Vector3.new(0, 0.24, 0)) * CFrame.Angles(0, 0, math.rad(90))

        -- Лопасть 1
        local blade1 = Instance.new("Part")
        blade1.Name = "Blade1_" .. i
        blade1.Size = Vector3.new(1.85, 0.025, 0.14)
        blade1.Color = LIGHT
        blade1.Material = Enum.Material.SmoothPlastic
        blade1.Transparency = 0.08
        blade1.Anchored = true
        blade1.CanCollide = false
        blade1.Massless = true
        blade1.Parent = model
        blade1.CFrame = topBody.CFrame * CFrame.new(off + Vector3.new(0, 0.32, 0))

        -- Лопасть 2
        local blade2 = Instance.new("Part")
        blade2.Name = "Blade2_" .. i
        blade2.Size = Vector3.new(0.14, 0.025, 1.85)
        blade2.Color = LIGHT
        blade2.Material = Enum.Material.SmoothPlastic
        blade2.Transparency = 0.08
        blade2.Anchored = true
        blade2.CanCollide = false
        blade2.Massless = true
        blade2.Parent = model
        blade2.CFrame = topBody.CFrame * CFrame.new(off + Vector3.new(0, 0.32, 0))

        -- Кольцо-размытие
        local blur = Instance.new("Part")
        blur.Name = "BladeBlur" .. i
        blur.Shape = Enum.PartType.Cylinder
        blur.Size = Vector3.new(0.015, 1.8, 1.8)
        blur.Material = Enum.Material.Neon
        blur.Color = Color3.fromRGB(180, 190, 200)
        blur.Transparency = 0.88
        blur.Anchored = true
        blur.CanCollide = false
        blur.Massless = true
        blur.Parent = model
        blur.CFrame = topBody.CFrame * CFrame.new(off + Vector3.new(0, 0.33, 0)) * CFrame.Angles(0, 0, math.rad(90))

        table.insert(Drone.props, {
            core = topBody,
            blades = {blade1, blade2},
            blur = blur,
            baseLocalCF = CFrame.new(off + Vector3.new(0, 0.32, 0)),
            baseLocalBlurCF = CFrame.new(off + Vector3.new(0, 0.33, 0)) * CFrame.Angles(0, 0, math.rad(90)),
        })
    end

    -- ============ LED-ОГНИ ============
    for _, pos in ipairs({ Vector3.new( 0.55, 0.05, -1.35), Vector3.new(-0.55, 0.05, -1.35) }) do
        local led = Instance.new("Part")
        led.Shape = Enum.PartType.Ball
        led.Size = Vector3.new(0.12, 0.12, 0.12)
        ...
        led.Massless = true
        led.Parent = model
        led.CFrame = topBody.CFrame * CFrame.new(pos)
    end
    for _, pos in ipairs({ Vector3.new( 0.55, 0.05, 1.35), Vector3.new(-0.55, 0.05, 1.35) }) do
        local led = Instance.new("Part")
        led.Shape = Enum.PartType.Ball
    ...
    -- ============ АНТЕННЫ (2 по бокам) ============
    for i, pos in ipairs({ Vector3.new( 0.75, 0.4, 0.9), Vector3.new(-0.75, 0.4, 0.9) }) do
        local ant = Instance.new("Part")
        ant.Name = "Antenna" .. i
        ant.Size = Vector3.new(0.035, 0.4, 0.035)
        ant.Color = DARKER
        ant.Material = Enum.Material.SmoothPlastic
        ant.Anchored = true
        ant.CanCollide = false
        ant.Massless = true
        ant.Parent = model
        ant.CFrame = topBody.CFrame * CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(i == 1 and -20 or 20))

        local tip = Instance.new("Part")
        tip.Name = "AntennaTip" .. i
        tip.Shape = Enum.PartType.Ball
        tip.Size = Vector3.new(0.09, 0.09, 0.09)
        tip.Material = Enum.Material.Neon
        tip.Color = Color3.fromRGB(255, 180, 40)
        tip.Anchored = true
        tip.CanCollide = false
        tip.Massless = true
        tip.Parent = model
        tip.CFrame = topBody.CFrame * CFrame.new(pos.X + (i == 1 and -0.14 or 0.14), pos.Y + 0.22, pos.Z)
    end
...
function Drone:Start()
    ...
    -- инициализация углов камеры от текущего положения
    ...
    -- ===== ВАЖНО: Разрешаем Roblox вращать камеру свайпом =====
    -- Мы НЕ отключаем UserInputService, оставляем всё как есть —
    -- но камеру будем контролировать сами. Для мобильного ввода
    -- мы будем читать MouseDelta через UserInputService.
    pcall(function()
...
-- ==========================================================
--  МОБИЛЬНОЕ УПРАВЛЕНИЕ КАМЕРОЙ
--  На телефоне — читаем свайп по правой половине экрана
--  и вращаем камеру вручную. На ПК — mouselock ведёт Roblox сам,
--  мы лишь читаем Camera.CFrame.
-- ==========================================================
local MobileCamControl = {
    active = false,
    lastPos = nil,
    touchInput = nil,
    sens = 0.005,       -- чувствительность для телефона
}

-- Ловим touch в правой половине экрана — это область камеры в Roblox
trackConn(UserInputService.TouchStarted:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if not Drone.active then return end
    -- правая половина экрана (там где Roblox обычно крутит камеру)
...
-- ==========================================================
--  DRONE FLIGHT — нативное управление Roblox
-- ==========================================================
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    -- ============ КАМЕРА ============
    -- На ПК: используем Camera.CFrame как есть (Roblox крутит через mouselock)
    -- На мобилке: наш camYaw / camPitch
    local camCF
    if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
        -- Собираем CFrame камеры из наших yaw/pitch
        local yaw = Drone.camYaw
        local pitch = Drone.camPitch

        -- Позиция камеры относительно дрона
        local camOffset = (Settings.DroneCamera == "FPV") and 0 or Settings.DroneCameraBack
        local basePos = Drone.droneCF.Position
        local camPos

        if Settings.DroneCamera == "FPV" then
            camPos = Drone.droneCF * CFrame.new(0, 0.35, -0.6).Position
        else
            local offset = CFrame.Angles(0, yaw, 0) * CFrame.Angles(pitch, 0, 0) * CFrame.new(0, 0, camOffset)
            camPos = basePos + offset.Position + Vector3.new(0, camOffset * 0.35, 0)
        end

        local lookDir = CFrame.Angles(0, yaw, 0) * CFrame.Angles(pitch, 0, 0) * Vector3.new(0, 0, -1)
        local target = basePos
        if Settings.DroneCamera == "FPV" then
            target = camPos + lookDir * 100
        end

        camCF = CFrame.lookAt(camPos, target)
    else
        -- ПК: берём Camera.CFrame, но для FPV всё равно пересобираем
        if Settings.DroneCamera == "FPV" then
            camCF = Drone.droneCF * CFrame.new(0, 0.35, -0.4)
        else
            camCF = Camera.CFrame
        end
    end
    ...
    if moveInput.Magnitude > 0.05 then
        ...
        moveDir = (forward * -moveInput.Z) + (right * moveInput.X)
    end
    ...
    if Settings.DroneCamera == "FPV" then
        ...
        local strafe = 0
        if moveInput.X > 0.3 then strafe = 1
        elseif moveInput.X < -0.3 then strafe = -1 end
        local targetRoll = math.rad(strafe * 4)
        Drone.fpvRoll = Drone.fpvRoll + (targetRoll - Drone.fpvRoll) * 0.15

        local camBaseCF = Drone.droneCF * CFrame.new(0, 0.35, -0.5)
...
hudCore.Color = Color3.fromRGB(28, 30, 36)
...
hudStripe.Size = Vector3.new(1.42, 0.05, 0.14)
...
hudStripe.CFrame = hudCore.CFrame * CFrame.new(0, 0.21, 0)
hudStripe.Parent = hudModel

local hudBlades = {}
for i = 1, 4 do
    local off = ({
        Vector3.new( 1.45, 0.35,  1.45),
        Vector3.new(-1.45, 0.35,  1.45),
        Vector3.new( 1.45, 0.35, -1.45),
        Vector3.new(-1.45, 0.35, -1.45),
    })[i]
    local blade = Instance.new("Part")
    blade.Size = Vector3.new(1.7, 0.025, 0.14)
    blade.Color = Color3.fromRGB(200, 202, 210)
        ...
        if Settings.ChatFlood and Settings.FloodText ~= "" then
            pcall(function()
                if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
                    ...
                    game:GetService("ReplicatedStorage").DefaultDefaultChatSystemChatEvents
