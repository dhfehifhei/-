-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition
-- ==========================================================
...
-- ==========================================================
--  ESP / Crosshair / Watermark / BulletTracers GUI
-- ==========================================================
...
local function sanitizeName(name)
    ...
    if #name == 0 then name = "config_" .. tostring(math.random(1000, 9999)) end
...
local function serializeSettings()
    ...
    for k, v in pairs(Settings) do
        ...
        elseif typeof(v) == "Vector2" then
            out[k] = { __vector2 = true, x = v.X, y = v.Y }
        else
            out[k] = v
        end
    end
...
local function deserializeSettings(tbl)
    for k, v in pairs(tbl) do
        ...
        elseif type(v) == "table" and v.__vector2 then
            Settings[k] = Vector2.new(v.x, v.y)
        else
            Settings[k] = v
        end
    end
...
local function saveConfigAs(name)
    ...
    if not ok then
        WindUI:Notify({ Title = "MellHack", Content = "Encode failed", Icon = "x", Duration = 4 })
        return false
    end
...
local function loadConfigByName(name)
    ...
    local path = CONFIG_DIR .. "/" .. sanitizeName(name) .. ".json"
    if not isfile(path) then
        WindUI:Notify({ Title = "MellHack", Content = "Not found: " .. name, Icon = "x", Duration = 3 })
        return false
    end
    local ok, data = pcall(function() return HttpService:JSONDecode(readfile(path)) end)
    if not ok or not data then
        WindUI:Notify({ Title = "MellHack", Content = "Read failed", Icon = "x", Duration = 3 })
        return false
    end
    local settings = data.settings or data
    deserializeSettings(settings)
    ApplyMenuColorsToTheme()
    WindUI:Notify({ Title = "MellHack", Content = "Loaded: " .. name, Icon = "check", Duration = 3 })
    return true
end
...
local function deleteConfigByName(name)
    ...
    pcall(delfile, path)
    WindUI:Notify({ Title = "MellHack", Content = "Deleted: " .. name, Icon = "x", Duration = 3 })
    return true
end
...
local function exportConfigToClipboard(name)
    if not (isfile and readfile and setclipboard) then
    ...
    WindUI:Notify({ Title = "MellHack", Content = "Copied to clipboard: " .. name, Icon = "check", Duration = 3 })
end

local function importConfigFromString(json, saveAs)
    ...
    ensureDirs()
    local name = sanitizeName(saveAs or (data.__meta and data.__meta.name) or ("imported_" .. os.time()))
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    if not data.__meta then
        data = { __meta = { name = name, created = os.time(), author = "imported", version = "3.0" }, settings = data }
    end
    local ok2, json2 = pcall(function() return HttpService:JSONEncode(data) end)
    if not ok2 then
        WindUI:Notify({ Title = "MellHack", Content = "Re-encode failed", Icon = "x", Duration = 4 })
        return false
    end
    pcall(writefile, path, json2)
    WindUI:Notify({ Title = "MellHack", Content = "Imported: " .. name, Icon = "check", Duration = 3 })
    return true
end

-- Функция прямого импорта строки от пользователя (через WindUI Input в Extras)
local function importFromClipboard()
    local clip
    if getclipboard then
        pcall(function() clip = getclipboard() end)
    elseif toclipboard then
        -- fallback — не читается, только пишется
    end
    if not clip or #clip < 10 then
        WindUI:Notify({ Title = "MellHack", Content = "Clipboard empty or unavailable", Icon = "x", Duration = 4 })
        return false
    end
    return importConfigFromString(clip, nil)
end
...
-- ==========================================================
--  Anti-OBS / Hide from recording
-- ==========================================================
local function applyHideFromRecording(state)
    Settings.HideFromRecording = state
    pcall(function()
        if WindUI and WindUI.ScreenGui then
            WindUI.ScreenGui.Enabled = not state
        end
        if WindUI and WindUI.NotificationGui then
            WindUI.NotificationGui.Enabled = not state
        end
        if WindUI and WindUI.DropdownGui then
            WindUI.DropdownGui.Enabled = not state
        end
        if WindUI and WindUI.TooltipGui then
            WindUI.TooltipGui.Enabled = not state
        end
        if ESPGui then
            ESPGui.Enabled = not state
        end
        for _, plr in pairs(Players:GetPlayers()) do
            if plr.Character then
                local hl = plr.Character:FindFirstChild("MellChams")
                if hl then hl.Enabled = not state end
            end
        end
    end)
end
...
--  Config tab
-- ==========================================================
local ConfigNameBuffer = ""
local ConfigListCache = {}

local function refreshConfigDropdown()
    ConfigListCache = listConfigs()
    return ConfigListCache
end
refreshConfigDropdown()

TabConfig:Section({ Title = "Save / Load" })
TabConfig:Input({
    Title = "Config name",
    Placeholder = "myconfig",
    Callback = function(text) ConfigNameBuffer = text end,
})
TabConfig:Button({
    Title = "Save Config",
    Icon = "save",
    Callback = function()
        local name = ConfigNameBuffer
        if not name or #name == 0 then
            name = "config_" .. tostring(os.time())
        end
...
TabConfig:Button({
    ...
    Icon = "folder-open",
    Callback = function()
        if ConfigNameBuffer and #ConfigNameBuffer > 0 then
            loadConfigByName(ConfigNameBuffer)
        else
            WindUI:Notify({ Title = "MellHack", Content = "Enter name first", Icon = "x", Duration = 3 })
        end
    end,
})
TabConfig:Button({
    ...
    Callback = function()
        if ConfigNameBuffer and #ConfigNameBuffer > 0 then
            deleteConfigByName(ConfigNameBuffer)
            refreshConfigDropdown()
        end
    end,
})
...
TabConfig:Button({
    ...
    Icon = "refresh-cw",
    Callback = function()
        local list = refreshConfigDropdown()
        WindUI:Notify({ Title = "MellHack", Content = "Found: " .. tostring(#list), Icon = "check", Duration = 3 })
    end,
})

-- Динамически добавляем по кнопке за каждую найденную конфигу
do
    for _, name in ipairs(ConfigListCache) do
        TabConfig:Button({
            Title = "Load: " .. name,
            Icon = "folder-open",
            Callback = function() loadConfigByName(name) end,
        })
    end
end

TabConfig:Section({ Title = "Export / Import" })
TabConfig:Button({
    Title = "Export to Clipboard",
    Icon = "copy",
    Callback = function()
        if ConfigNameBuffer and #ConfigNameBuffer > 0 then
            exportConfigToClipboard(ConfigNameBuffer)
        else
            WindUI:Notify({ Title = "MellHack", Content = "Enter name first", Icon = "x", Duration = 3 })
        end
    end,
...
TabConfig:Input({
    ...
    Callback = function(text)
        if text and #text > 10 then
            importConfigFromString(text, nil)
            refreshConfigDropdown()
        end
    end,
})
...
TabConfig:Input({
    ...
    Placeholder = "myconfig",
    Callback = function(text)
        setAutoLoadName(text)
        WindUI:Notify({ Title = "MellHack", Content = "Auto-load: " .. tostring(text), Icon = "check", Duration = 3 })
    end,
})
TabConfig:Button({
...
TabBinds:Keybind({ Title = "Hide from Recording", Value = "F1", Callback = function()
    applyHideFromRecording(not Settings.HideFromRecording)
end })
...
TabSettings:Section({ Title = "Recording" })
TabSettings:Toggle({ Title = "Hide from Recording (OBS)", Value = false,
    Callback = function(v) applyHideFromRecording(v) end })
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        if Settings.Chams then
            if not char:FindFirstChild("MellChams") then
                ...
                hl.Enabled = not Settings.HideFromRecording
            else
