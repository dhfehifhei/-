-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v4
-- ==========================================================
...
local function copyConfigToClipboard(name)
    ...
    WindUI:Notify({ Title = "MellHack", Content = "Clipboard unavailable", Icon = "x", Duration = 3 })
    return false
end

local function importConfigFromJSON(json, saveAs)
    if not (writefile and isfolder) then
        WindUI:Notify({ Title = "MellHack", Content = "writefile unavailable", Icon = "x", Duration = 3 })
        return false
    end
    ...
    if saveAs and #saveAs > 0 then
        ...
        if configExists(name) then
            name = name .. "_" .. tostring(os.time() % 10000)
        end
    else
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "4.0" }
        payload.__meta.name = name
    else
        payload = {
            __meta = { name = name, created = os.time(), version = "4.0", author = "imported" },
            settings = data,
        }
    end
    local encOk, encoded = pcall(function() return HttpService:JSONEncode(payload) end)
    if not encOk then
        WindUI:Notify({ Title = "MellHack", Content = "Encode failed", Icon = "x", Duration = 3 })
        return false
    end
...
-- ==========================================================
--  Recording (screenshots + video)
-- ==========================================================
local RECORD_DIR = CONFIG_ROOT .. "/videos"
local SHOT_DIR   = CONFIG_ROOT .. "/screenshots"

local function ensureMediaDirs()
    if not isfolder then return end
    ensureDirs()
    if not isfolder(RECORD_DIR) then pcall(makefolder, RECORD_DIR) end
    if not isfolder(SHOT_DIR) then pcall(makefolder, SHOT_DIR) end
end
ensureMediaDirs()

local recordingActive = false
local recordingFunctions = {}
local hiddenGuis = {}

local function hideAllScriptGUIs()
    hiddenGuis = {}
    local function hide(obj)
        if obj and obj:IsA("ScreenGui") and obj.Enabled ~= false then
            table.insert(hiddenGuis, obj)
            obj.Enabled = false
        end
    end
    hide(WindUI and WindUI.ScreenGui)
    hide(WindUI and WindUI.NotificationGui)
    hide(WindUI and WindUI.DropdownGui)
    hide(WindUI and WindUI.TooltipGui)
    hide(ESPGui)
    for _, plr in pairs(Players:GetPlayers()) do
        if plr.Character then
            local hl = plr.Character:FindFirstChild("MellChams")
            if hl then hl.Enabled = false end
        end
    end
end

local function showAllScriptGUIs()
...
local function tryStartVideoRecording()
    local path = RECORD_DIR .. "/rec_" .. tostring(os.time()) .. ".mp4"

    -- Delta
    if Delta and Delta.Record then
        local ok, res = pcall(function() return Delta.Record(path) end)
        if ok and res ~= nil then
            table.insert(recordingFunctions, function()
                pcall(function() if Delta.StopRecord then Delta.StopRecord() end end)
            end)
            return true, path
        end
    end
    ...
    -- Synapse
    if syn and syn.record then
        local ok, res = pcall(function() return syn.record(path) end)
        if ok and res ~= nil then
            table.insert(recordingFunctions, function()
                pcall(function() if syn.stoprecord then syn.stoprecord() end end)
            end)
            return true, path
        end
    end

    -- startRecord / stopRecord (некоторые экзекуторы)
    if startRecord then
        local ok, res = pcall(startRecord, path)
        if ok then
            table.insert(recordingFunctions, function()
                pcall(function() if stopRecord then stopRecord() end end)
            end)
            return true, path
        end
    end
    ...
    if startRecording then
        local ok, res = pcall(startRecording, path)
        if ok then
            table.insert(recordingFunctions, function()
                pcall(function() if stopRecording then stopRecording() end end)
            end)
            return true, path
        end
    end
...
local function tryStopVideoRecording()
    for i = #recordingFunctions, 1, -1 do
        pcall(recordingFunctions[i])
        recordingFunctions[i] = nil
    end
end

local function takeScreenshot()
    ensureMediaDirs()
    local path = SHOT_DIR .. "/shot_" .. tostring(os.time()) .. ".png"

    local hadWindUI = WindUI and WindUI.ScreenGui
    local hadESP = ESPGui
    local wasEnabled = {}
    if hadWindUI then wasEnabled.WindUI = hadWindUI.Enabled; hadWindUI.Enabled = false end
    if hadESP then wasEnabled.ESP = hadESP.Enabled; hadESP.Enabled = false end

    task.wait(0.05)

    local saved = false
    local function restore()
        if hadWindUI and wasEnabled.WindUI ~= nil then hadWindUI.Enabled = wasEnabled.WindUI end
        if hadESP and wasEnabled.ESP ~= nil then hadESP.Enabled = wasEnabled.ESP end
    end

    if syn and syn.takescreenshot then
        local ok = pcall(syn.takescreenshot, path)
        saved = ok
    elseif Delta and Delta.Screenshot then
        local ok = pcall(Delta.Screenshot, path)
        saved = ok
    elseif takescreenshot then
        local ok = pcall(takescreenshot, path)
        saved = ok
    elseif screenshot then
        local ok = pcall(screenshot, path)
        saved = ok
    end
    ...
    if saved then
        WindUI:Notify({ Title = "MellHack", Content = "Screenshot: " .. path, Icon = "check", Duration = 4 })
    else
        WindUI:Notify({ Title = "MellHack", Content = "Screenshot API unavailable", Icon = "x", Duration = 4 })
    end
    return saved
end
...
local function startRecording()
    if recordingActive then
        WindUI:Notify({ Title = "MellHack", Content = "Already recording", Icon = "x", Duration = 3 })
        return
    end
    hideAllScriptGUIs()
    local ok, path = tryStartVideoRecording()
    if ok then
        recordingActive = true
        WindUI:Notify({ Title = "MellHack", Content = "Recording started (UI hidden)", Icon = "check", Duration = 4 })
    else
        showAllScriptGUIs()
        WindUI:Notify({ Title = "MellHack", Content = "Recorder API unavailable", Icon = "x", Duration = 4 })
    end
end

local function stopRecording()
    if not recordingActive then
        WindUI:Notify({ Title = "MellHack", Content = "Not recording", Icon = "x", Duration = 3 })
        return
    end
    tryStopVideoRecording()
    recordingActive = false
    showAllScriptGUIs()
    WindUI:Notify({ Title = "MellHack", Content = "Recording saved to " .. RECORD_DIR, Icon = "check", Duration = 4 })
end

local function takeScreenshotNow()
    takeScreenshot()
end
...
local TabMedia    = Window:Tab({ Title = "Recorder",   Icon = "camera" })
...
-- ==========================================================
--  Configs tab (v4 — list-based)
-- ==========================================================
local renameBuffer = ""
local importBuffer = ""

local function rebuildConfigList()
    -- WindUI не даёт удалять элементы из таба, поэтому список
    -- пересобираем через отдельные Section с кнопками.
    -- Вместо удаления — используем Notify, что список обновится
    -- при следующем запуске скрипта. Но чтобы UX был норм:
    -- просто показываем текущий список в Notify + даём действовать.
...
TabConfig:Input({
    Title = "New config name (leave empty for MellHack_NN)",
...
TabConfig:Button({
    ...
    Callback = function()
        ...
        if saveConfigAs(name) then
            ...
            WindUI:Notify({ Title = "MellHack", Content = "Save failed", Icon = "x", Duration = 3 })
        end
    end,
})
TabConfig:Button({
    ...
    Callback = function()
        ...
        if importConfigFromJSON(clip, nil) then
            rebuildConfigList()
        end
    end,
})
...
TabConfig:Button({
    ...
    Callback = function()
        ...
        if importConfigFromJSON(importBuffer, nil) then
            rebuildConfigList()
        end
    end,
})
...
TabConfig:Button({
    ...
    Callback = function()
        if not renameBuffer or #renameBuffer == 0 then
            WindUI:Notify({ Title = "MellHack", Content = "Type name above first", Icon = "x", Duration = 3 })
            return
        end
        if loadConfigByName(renameBuffer) then
            WindUI:Notify({ Title = "MellHack", Content = "Loaded: " .. renameBuffer, Icon = "check", Duration = 3 })
        else
            WindUI:Notify({ Title = "MellHack", Content = "Not found: " .. renameBuffer, Icon = "x", Duration = 3 })
        end
    end,
})
TabConfig:Button({
    ...
    Icon = "edit",
    Callback = function()
        WindUI:Notify({ Title = "MellHack", Content = "Type OLD name above, then use Notify to confirm — UI rename через кнопку ниже", Icon = "info", Duration = 4 })
    end,
})
TabConfig:Input({
    Title = "Rename: new name",
    Placeholder = "new name",
    Callback = function(text) importBuffer = text end, -- reused
})
TabConfig:Button({
    ...
    Callback = function()
        ...
        if not importBuffer or #importBuffer == 0 then
            WindUI:Notify({ Title = "MellHack", Content = "Type NEW name in 'Rename: new name' field", Icon = "x", Duration = 3 })
            return
        end
        if renameConfig(renameBuffer, importBuffer) then
            WindUI:Notify({ Title = "MellHack", Content = "Renamed: " .. renameBuffer .. " → " .. importBuffer, Icon = "check", Duration = 3 })
            rebuildConfigList()
        else
...
-- Динамически рисуем по кнопке на каждую существующую конфигу,
-- с действиями Load / Copy / Delete. Если конфиг добавили позже —
-- нужно перезапустить скрипт, чтобы кнопки обновились.
do
    local list = listConfigs()
    if #list > 0 then
...
TabMedia:Button({
    ...
    Icon = "camera",
    Callback = takeScreenshotNow,
})

TabMedia:Section({ Title = "Info" })
TabMedia:Button({
    Title = "Show Save Paths",
    Icon = "folder",
    Callback = function()
        WindUI:Notify({
            Title = "MellHack",
            Content = "Videos: " .. RECORD_DIR .. " | Screens: " .. SHOT_DIR,
            Icon = "folder",
            Duration = 6,
        })
    end,
...
TabBinds:Keybind({ Title = "Start/Stop Recording", Value = "F2", Callback = function()
    if recordingActive then stopRecording() else startRecording() end
end })
TabBinds:Keybind({ Title = "Screenshot", Value = "F3", Callback = takeScreenshotNow })
...
getgenv().MellHackUnload = function()
    ...
    if recordingActive then
        pcall(tryStopVideoRecording)
        recordingActive = false
        pcall(showAllScriptGUIs)
    end
