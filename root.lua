-- ==========================================================
--  MellHack // @ruzoxu  |  Delta Mobile Edition v7
-- ==========================================================
...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "7.0" },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "7.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "7.0" }, settings = data }
    end
...
-- ==========================================================
--  DRONE
--  Физика отключена полностью. Дрон — anchored Part,
--  двигаем через CFrame вручную. Никакой тряски.
-- ==========================================================
local Drone = {
    model        = nil,
    base         = nil,
...
local function buildDroneModel()
    ...
    -- Core body (ANCHORED — не двигается сам, только через скрипт)
    ...
    for i, off in ipairs(propOffsets) do
        ...
        table.insert(Drone.props, {
            ...
            blur = blur,
            baseLocalCF = CFrame.new(off + Vector3.new(0, 0.28, 0)),
            baseLocalBlurCF = CFrame.new(off + Vector3.new(0, 0.3, 0)) * CFrame.Angles(0, 0, math.rad(90)),
        })
    end

    -- Сохраняем смещения всех частей относительно Core
    model.DescendantAdded:Connect(function() end)

    return model
end

-- Перемещает всю модель (все anchored части) в заданный CFrame
...
function Drone:Start()
    ...
    -- переносим всю модель в self.droneCF
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    ...
    if Drone.currentVel.Magnitude > 0.1 then
        ...
        if hit then
            ...
            -- slight pushback so we don't stick inside wall
            local back = Drone.currentVel.Unit * 0.5
            Drone.droneCF = Drone.droneCF + back
        else
            Drone.droneCF = Drone.droneCF + step
        end
    end

    -- Orientation: face movement direction, small pitch when moving
    local lookDir
    ...
    if Settings.DroneCamera == "FPV" then
        ...
        local behind = (Drone.droneCF.LookVector * -camOffset) + Vector3.new(0, camOffset * 0.4, 0)
...
-- Propeller spin: вращаем только лопасти (они anchored, крутим их локально)
